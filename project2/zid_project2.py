...
def read_prc_csv(tic):
    ...
    """
    df=pd.read_csv(os.path.join(cfg.DATADIR,f"{tic.lower()}_prc.csv"),index_col=0)
...
def mk_prc_df(tickers, prc_col='adj_close'):
    ...
    **not** necessarily represent the actual contents of the data frame you
    ...
    """
    prc_df = pd.concat([read_prc_csv(i)[prc_col].rename(i) for i in tickers], axis=1, sort=True, names="Date")
    prc_df.index.name = "Date"
    return prc_df
