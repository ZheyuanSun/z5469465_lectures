...
def get_tics(pth):
    ...
    """
    # <COMPLETE THIS PART>
    with open(pth) as file_on:
        tickss= {}
        for cycle in file_on:
            tickss[cycle.split('"')[3].lower()]=cycle.split('"')[1].lower()
    return tickss
...
def read_dat(tic):
    ...
    # <COMPLETE THIS PART>
    tic_path = os.path.join(DATDIR, f"{tic}_prc.dat")
    with open(tic_path) as file_on:
        cycle = []
        for line in file_on:
            cycle.append(line.split('/n')[0].lower())
    return cycle
...
def create_data_dict(tic_exchange_dic, tickers_lst=None, col_lst=None):
    ...
    for ticker in tickers_lst:
        ...
        for line in read_dat(ticker):
            ...
            filtered_dict = {key: line_dict[key] for key in col_lst}
